 ...
 * piping it through `awk`, which wraps every line in ANSI truecolor escapes to
 * form a vertical brand gradient. The palette is chosen to match the terminal
 * theme (light vs. dark) so the banner stays legible on either background.
 *
 ...
 */
function colorize(mode: "light" | "dark", lineCount: number): string {
